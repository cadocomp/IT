<#
    ...
    << Rob Gray merged additions public folder processing
			28 Sept2021																		   
	>>		
    ...
    ----<<RG2>>---------
    .\RemDup188rg2.ps1 -Identity theBoss -PublicFolders -PFStart "_Projects\2005" -Credentials $Credentials -Impersonation -Report -Force
    ...
    [PS] C:\WINDOWS\system32>New-ManagementRoleAssignment -name:impersonationAssignmentName -Role:ApplicationImpersonation -User:TheBoss
    --------------------
...
param(
    ...
    [parameter( Mandatory= $false, ParameterSetName= 'OAuthCertSecretArchiveOnly')] 
    [parameter( Mandatory= $false, ParameterSetName= 'BasicAuthArchiveOnly')] 
    [parameter(Mandatory=$False,HelpMessage="If this item is present, public folders searched starting folder path, not root", ParameterSetName = "PubFolders")]   
    [string]$PFStart, 
    ...
    Function myEWSFind-Folders {
        ...
        Do {
            Try {
                ...
                $OpSuccess= $false
                $critErr= $true
                Write-Warning ('Error performing operation FindFolders with Search options in {0}.\n Error: {1}' -f $FolderId.FolderName, $Error[0])
                Write-Warning (' --FolderId>>{0}<<' -f $FolderId.UniqueId)																		  
            }
            finally {
...
#region RobGray-1	
	function GetFolder 	{ 
		...
		Write-Verbose('...finding {0}' -f $FolderPath)
		$origPath = $FolderPath
		try 
		{ 
			if ($PublicFolders) 
		...
		Write-Verbose('...found root {0}' -f $Folder.DisplayName)
	    $fnd = $false
		if ($FolderPath -ne '\') 
		{ 
            ...
            { 
				...
				Write-Host ('Finding subfolder..{0}  {1}' -f $Folder.DisplayName,$value)
				$FolderView= New-Object Microsoft.Exchange.WebServices.Data.FolderView( 100)
				$FolderView.Traversal= [Microsoft.Exchange.WebServices.Data.FolderTraversal]::Shallow
				
				Write-Verbose('Folderview created :{0} {1}' -f $FolderView.PageSize,$FolderView.PropertySet)
				...
				ForEach ( $FolderItem in $FolderSearchResults) {
					$FolderPath= '{0}\{1}' -f $Folder.DisplayName, $FolderItem.DisplayName
					Write-Verbose('search={0}' -f $Folderpath)
					if($FolderItem.DisplayName -eq $value) {
						$Folder = $FolderItem;
						$fnd = $true;
						Write-Verbose('Found={0}' -f $Folderpath)
						break;
					}
				}
		...
		Else
		{
			Write-Output ('Failed to find: {0}' -f $origPath ) 
			Write-Verbose ('Failed to find: {0}' -f $origPath ) 
			$res = $null
		}
        ...
        ForEach ( $SubFolder in $FoldersToProcess) {
            ...
            If ( ! ( $DeleteMode -eq 'MoveToDeletedItems' -and $SubFolder.Folder.Id -eq $DeletedItemsFolder.Id)) {
                ...
                $ItemView.PropertySet= New-Object Microsoft.Exchange.WebServices.Data.PropertySet( [Microsoft.Exchange.WebServices.Data.BasePropertySet]::FirstClassProperties)
						...
						$ItemView.PropertySet.Add( [Microsoft.Exchange.WebServices.Data.FolderSchema]::DisplayName)
						$ItemView.PropertySet.Add( [Microsoft.Exchange.WebServices.Data.FolderSchema]::FolderClass)
                        ...
                        ForEach ( $Item in $ItemSearchResults.Items) {
                            ...
                            If ( $null -ne $key) {
                                ...
                                If ( $global:UniqueList.contains( $hash)) {
                                    ...
                                    $null= $DuplicateList.Add( $Item.Id)
                                    $TotalDuplicates++
                                }
                                Else {
    ...
    #<<RG increase min EX versions. cant recall why !
    If ( $MailboxOnly) {
...
Process {
	...
	#<<RG test PF option
        if( $PublicFolders ) {    
           Write-Host ('Processing Public folders [{0}] as {1} ({2}) ' -f $PFStart, $EmailAddress, $CurrentIdentity)
		}
        else  {
        ...
        $PriorityFilter= Construct-FolderFilter $EwsService $PriorityFolders $EmailAddress
 		...
 		##<< robg  get root and start folder
		if ( $PublicFolders.IsPresent) {
		    #try {
                ...
                If ( $RootFolder) {
                    ...
                    if($RootFolder -eq $null -Or [string]::IsNullOrEmpty($RootFolder.DisplayName))
                    {
                        Write-Error ('Problem accessing {0}' -f $PFStart)
                        Exit $ERR_PROCESSINGMAILBOX
                    }

					Write-Verbose ('Rootfolders {0} <<{1}>>' -f $PFStart,$RootFolder.DisplayName)
